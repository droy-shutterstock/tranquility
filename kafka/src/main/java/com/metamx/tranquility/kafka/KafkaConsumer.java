...
public class KafkaConsumer {
    ...
    private final StringDecoder keyDecoder;
    ...
    public KafkaConsumer(
        ...
        VerifiableProperties vProps = new VerifiableProperties(kafkaProperties);
        keyDecoder = new StringDecoder(vProps);
    ...
    private void startConsumers() {
        final List<KafkaStream<String, Object>> kafkaStreams = consumerConnector.createMessageStreamsByFilter(
                topicFilter, numThreads, keyDecoder, valueDecoder
        );

        for (final KafkaStream<String, Object> kafkaStream : kafkaStreams) {
            consumerExec.submit(
                    new Runnable() {
                        @Override
                        public void run() {
                            try {
                                final Iterator<MessageAndMetadata<String, Object>> kafkaIterator = kafkaStream.iterator();
